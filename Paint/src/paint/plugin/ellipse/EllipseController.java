...
public class EllipseController extends AnchorPane implements CommandPane {
	...
	public void execute(Object canvas, MouseEvent event) {
		if (event.getEventType() == (MouseEvent.MOUSE_PRESSED)) {
			...
			if (event.getX() + 20 >= parent.getBoundsInLocal().getWidth() || event.getX() - 20 <= 0
					|| event.getY() + 20 >= parent.getBoundsInLocal().getHeight() || event.getY() - 20 <= 0)
				return;
	...
	public EllipseController() {
		...
		//			+ "paint" + File.separator + "plugin" + File.separator + TOOL + File.separator + "ellipse.fxml"), this);
			uiLoader = new UILoader( getClass().getResource("ellipse.fxml"),this);
	        uiLoader.load();
    ...
    public void setAction(EventHandler<ActionEvent> value) {
        ...
        animation.addShapeIconAnimation(ellipseButton);
	}
